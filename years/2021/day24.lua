  ...
  for i = start ~= nil and start or 1, #prog do
    ...
    elseif prog[i].op == 'div' then
      if not prog[i].b_int then
        day24_reg_set(
          prog[i].a,
          day24_trunc(day24_reg_get(prog[i].a, registers) / day24_reg_get(prog[i].b, registers)),
          registers
        )
      else
    ...
    elseif prog[i].op == 'eql' then
      if not prog[i].b_int then
        day24_reg_set(
          prog[i].a,
          day24_trunc(day24_reg_get(prog[i].a, registers) == day24_reg_get(prog[i].b, registers) and 1 or 0),
          registers
        )
      else
