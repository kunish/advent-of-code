...
function day23_moves(index, state, movecache)
  ...
  for y1 = 2, (state.part1 and 3 or 5) do
    if
      state.positions[day23_pos_from_coord(room_x, y1)] ~= 0
      and state.positions[day23_pos_from_coord(room_x, y1)] ~= me
    then
      okroom_x = false
      break
    end
...
function day23_solve(state, movecache, part1)
  ...
  while #queue > 0 do
    ...
    if
      (finalcost == nil or finalcost > curr_state.cost)
      and (states[curr_state_key] == nil or (states[curr_state_key] > curr_state.cost))
    then
...
function day23(path)
  ...
  local positions = {
    string.sub(lines[3], 4, 4),
    string.sub(lines[3], 6, 6),
    string.sub(lines[3], 8, 8),
    string.sub(lines[3], 10, 10),
    string.sub(lines[4], 4, 4),
    string.sub(lines[4], 6, 6),
    string.sub(lines[4], 8, 8),
    string.sub(lines[4], 10, 10),
  }
